...
mod tests {
    ...
    use crate::ops::{
        GenerateOptions,
        generate::generate_with_log_n,
        verify::{PublicKeySource, VerifyOptions, verify},
    };
    use tempfile::TempDir;

    /// Minimum scrypt `log_n` for fast test key generation (still secure enough for tests)
    const TEST_KDF_LOG_N: u8 = 4;

    /// Generate a keypair with fast KDF parameters into `dir`, return (`sk_path`, `pk_path`).
    fn generate_test_keypair(dir: &TempDir) -> (std::path::PathBuf, std::path::PathBuf) {
        let sk = dir.path().join("test.key");
        let pk = dir.path().join("test.pub");
        let opts = GenerateOptions::builder(&sk, &pk)
            .no_password(true)
            .force(true)
            .build();
        generate_with_log_n(&opts, None, TEST_KDF_LOG_N).expect("keypair generation must succeed");
        (sk, pk)
    }

    #[test]
    fn prehashed_sign_verify_round_trip() {
        let dir = TempDir::new().expect("temp dir");

        let (sk, pk) = generate_test_keypair(&dir);

        let msg = dir.path().join("message.txt");
        std::fs::write(&msg, b"hello prehashed world").expect("write message");

        let sig = dir.path().join("message.txt.minisig");

        // Sign with prehashed=true
        let sign_opts = SignOptions::builder(&sk, &msg)
            .prehashed(true)
            .force(true)
            .quiet(true)
            .build();
        sign_single_file(&msg, &sign_opts, None).expect("prehashed sign must succeed");

        assert!(sig.exists(), "signature file must be created");

        // Verify the prehashed signature
        let verify_opts = VerifyOptions::builder(PublicKeySource::File(&pk), &sig, &msg)
            .quiet(true)
            .build();
        verify(&verify_opts).expect("prehashed verify must succeed");
    }
}
