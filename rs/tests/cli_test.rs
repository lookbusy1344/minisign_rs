...
#[test]
fn test_generate_hardware_key_flag_recognized() {
    // Test that --hardware-key flag is recognized by the CLI parser
    // On platforms without hardware key support, this will fail with
    // "hardware key store unavailable" rather than "unknown argument"
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let secret_key = temp_dir.path().join("hw_test.key");
    let public_key = temp_dir.path().join("hw_test.pub");
    let password_file = temp_dir.path().join("password.txt");

    fs::write(&password_file, "testpass").expect("Failed to write password");

    let result = minisign_cmd()
        .arg("-G")
        .arg("--hardware-key")
        .arg("-s")
        .arg(&secret_key)
    ...
    // The flag should be recognized (not "unknown argument" or "unexpected argument")
    assert!(
        !stderr.contains("unknown") && !stderr.contains("unexpected"),
        "Flag should be recognized, but got error: {stderr}"
    );

    // On most test platforms, hardware key store won't be available
    // So we expect either success OR a hardware-related error (not argument error)
    if !result.status.success() {
        // Should be a hardware availability error, not an argument parsing error
        assert!(
            stderr.contains("hardware") || stderr.contains("unavailable"),
            "Expected hardware-related error, got: {stderr}"
        );
    }
}

#[test]
fn test_generate_hardware_key_short_alias() {
    // Test that --hw alias works
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let secret_key = temp_dir.path().join("hw_alias_test.key");
    let public_key = temp_dir.path().join("hw_alias_test.pub");
    let password_file = temp_dir.path().join("password.txt");
    ...
    // The --hw alias should be recognized
    assert!(
        !stderr.contains("unknown") && !stderr.contains("unexpected"),
        "Flag alias should be recognized, but got error: {stderr}"
    );
}

#[test]
fn test_generate_hardware_key_requires_password() {
    // Test that hardware key generation requires a password (cannot use -W)
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let secret_key = temp_dir.path().join("hw_nopw_test.key");
    let public_key = temp_dir.path().join("hw_nopw_test.pub");

    minisign_cmd()
        .arg("-G")
        .arg("--hardware-key")
        .arg("-W") // No password - should be rejected
        .arg("-s")
        .arg(&secret_key)
        .arg("-p")
        .arg(&public_key)
...
#[test]
#[cfg(debug_assertions)]
fn test_generate_hardware_key_quiet_mode() {
    // Test that -q flag suppresses hardware key enrollment messages
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let secret_key = temp_dir.path().join("hw_quiet_test.key");
    let public_key = temp_dir.path().join("hw_quiet_test.pub");
    let password_file = temp_dir.path().join("password.txt");
    ...
    let result = minisign_cmd()
        ...
        .arg("--password-file")
        .arg(&password_file)
        .arg("--force-weak-kdf") // Speed up test
        .assert()
        .get_output()
        .clone();
    ...
    // In quiet mode, should not see "Enrolling" or "enrolled" messages
    // (even if hardware key store were available)
    // Note: We might still see error messages if hardware is unavailable
    if result.status.success() {
        assert!(
            !stderr.contains("Enrolling") && !stderr.contains("enrolled"),
            "Quiet mode should suppress enrollment messages, got: {stderr}"
        );
    }
}
