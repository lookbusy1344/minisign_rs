//! Unit tests for signature creation operations

use minisign::{
    crypto::{
        KeyNum, Signature, blake2b_512, blake2b_512_stream, generate_keypair,
...
fn test_sign_unencrypted_key() {
    ...
    let options = SignOptions {
        secret_key_file: PathBuf::from("tests/fixtures/keys/unencrypted.key"),
...
fn test_sign_encrypted_key() {
    ...
    let options = SignOptions {
        secret_key_file: PathBuf::from("tests/fixtures/keys/test.key"),
        message_file: message_path.clone(),
        signature_file: Some(sig_path.clone()),
        prehashed: true,
