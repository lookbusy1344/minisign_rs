...
fn test_unicode_in_trusted_comment() {
    ...
    let sign_opts = SignOptions::new(
        ...
        Some(sig_file.as_path()),
        true,
        Some("Signed 🔐 签名 توقيع"),
        Some("Test signature 测试 اختبار 🚀"),
        true,
        false,
    );
...
fn test_unicode_multibyte_at_byte_limit() {
    ...
    let sign_opts = SignOptions::new(
        ...
        Some(sig_file.as_path()),
        true,
        Some(comment_with_multibyte.as_str()),
        None,
        true,
        false,
    );
